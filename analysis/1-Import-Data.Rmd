...
GOAL: Check Cellbender impact

```{r}
# Master directory Cellbender
masterdir.bender <- "/shares/rheumatologie.usz/Gino/Laura/Cellbender_output"

benderfiles <- list.files(
  path = masterdir.bender, 
  pattern = "cellbender_output_Exp.*\\.h5", 
  full.names = TRUE, 
  recursive = TRUE
)

# removing sample Exp3_CMHEF_TGF as it somehow is corrupted.
benderfiles <- benderfiles[-6]
# removing sample Exp4_EC_diseased as the bender file shows 10x more cells??
benderfiles <- benderfiles[-12]

#########################################################################

# Master directory no Cellbender
masterdir.no.bender <- "/shares/rheumatologie.usz/Gino/Laura/Cellranger_output"

no.benderfiles <- list.files(
  path = masterdir.no.bender, 
  pattern = "Exp.*\\.h5", 
  full.names = TRUE, 
  recursive = TRUE
)
...
```{r qc-comparison-loop, fig.height=7, fig.width=8}
